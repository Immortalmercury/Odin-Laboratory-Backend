...
class APIController extends Controller
{
    public function index(Request $request)
    {
        switch ($request->method) {
            case 'get_lab_file':
                return $this->get_lab_file($request);
                break;
            case 'get_user_data':
                return $this->get_user_data($request);
                break;
            case 'get_full_user_data':
                return $this->get_full_user_data($request);
                break;
            case 'get_resource':
            ...
            case 'get_user_notifications':
                return $this->get_user_notifications($request);
                break;
            case 'get_new_user_notifications':
                return $this->get_new_user_notifications($request);
                break;

            default:
                switch ($request->jwt_user_role) {
                    case 'student':
                        $otherController = new StudentController();
                        return $otherController->index($request);
                        break;

                    case 'teacher':
                    case 'admin':
                        $otherController = new TeacherController();
                        return $otherController->index($request);
                        break;

                    default:
                        return self::error("UNDEFINED_METHOD (СC)");
                        break;
                }
                break;
        }
